// header
document.getElementById('header-container').innerHTML = `
<nav id="uni-navigation-bar" class="js-nav nav-02  nav-02--sticky  nav-02--sticky--black  ">
<div class="container container--large">
    <div class="nav-02__box">
        <div class="nav-02__logo"><a class="nav-02__link" href="./index.html" target="_self">
            
    
                <img class="nav-02__logo_img" height="39" alt="Frame 2376" src="./index_files/frame-2376.png" srcset="https://unicorn-cdn.b-cdn.net/f23ad5de-7146-4965-9d51-f32bec29011c/frame-2376.png?width=150&amp;height=39 150w,https://unicorn-cdn.b-cdn.net/f23ad5de-7146-4965-9d51-f32bec29011c/frame-2376.png?width=300&amp;height=78 300w,https://unicorn-cdn.b-cdn.net/f23ad5de-7146-4965-9d51-f32bec29011c/frame-2376.png?width=450&amp;height=117 450w," sizes="150px">
            
            
        </a></div>
        <div class="nav-02__links js-menu">
            <div class="nav-02__list_wrapper   nav-02__list_wrapper--black ">
                <ul class="nav-02__list nav-02__list--desktop">
                
                
                    




    
        <li class="nav-02__item">
            
                <a class="button  button--white-outline button--empty " href="./aboutus.html" target="_self"><span class="button__text">About Us</span>
                    </a>
            
        </li>
    



    
        <li class="nav-02__item">
            
                <a class="button  button--white-outline button--empty " href="./contactus.html" target="_self"><span class="button__text">Contact Us</span>
                    </a>
            
        </li>
    



                
                    <li class="nav-02__item">
                        

<div class="buttons-set"><ul class="buttons-set__list"><li class="buttons-set__item"><a data-stripe-product-id="" data-stripe-mode="payment" data-successful-payment-url="" data-cancel-payment-url="" class="button button--white-outline " href="https://tally.so/r/nro7xR" target="_blank"><span class="button__text">Get Early Access</span></a></li></ul></div>

                    </li>
                </ul>
                <ul class="nav-02__list nav-02__list--mobile">
                    
                    
                        




    
        <li class="nav-02__item">
            
                <a class="button  button--white-outline button--empty " href="./aboutus.html" target="_self"><span class="button__text">About Us</span>
                    </a>
            
        </li>
    



    
        <li class="nav-02__item">
            
                <a class="button  button--white-outline button--empty " href="./contactus.html" target="_self"><span class="button__text">Contact Us</span>
                    </a>
            
        </li>
    



                    
                    <li class="nav-02__item">
                        

<div class="buttons-set"><ul class="buttons-set__list"><li class="buttons-set__item"><a data-stripe-product-id="" data-stripe-mode="payment" data-successful-payment-url="" data-cancel-payment-url="" class="button button--white-outline " href="https://tally.so/r/nro7xR" target="_blank"><span class="button__text">Get Early Access</span></a></li></ul></div>

                    </li>
                </ul>
            </div>
  
    
      <div class="nav-02__burger">
<button class="burger burger--black js-open-menu" type="button">
<div class="burger__box">
    <div class="burger__inner"></div>
</div>
</button>
</div>
    
  
        </div>
    </div>
</div>
</nav>

<style>
#uni-navigation-bar {
background-color: #555 ;
}
</style>

`;


// footer
document.getElementById('footer-container').innerHTML = `
            
<div class="bg-custom-color">
<footer class="footer-04 footer-04--dark" id="footer" style="background-color: #555555;">
    <div class="footer-04__top">
        <div class="container">
            <div class="footer-04__top_wrapper  text-white ">
                
                    


<div class="footer-04__col">

<a class="footer-04__logo_link" href="./index.html" target="_self">

    


<img loading="lazy" alt="" class="footer-04__logo" height="35" src="./index_files/image.png">



</a>


<div class="footer-04__info content_box"><p>Copyright © 2023&nbsp; Ansmake Technologies LLP</p>
</div>

</div>


                
                
                    
                        <div class="footer-04__col">
                            <h3 class="footer-04__title">Company</h3>
                            <ul class="footer-04__list">
                                
                                    
                                        <li class="footer-04__item">
                                            <a href="./aboutus.html" target="_self" class="footer-04__link">
                                                About Us
                                            </a>
                                        </li>
                                    
                                
                                    
                                        <li class="footer-04__item">
                                            <a href="./contactus.html" target="_self" class="footer-04__link">
                                                Contact Us
                                            </a>
                                        </li>
                                    
                                
                                    
                                
                            </ul>
                        </div>
                    
                
                
                    <div class="footer-04__col">
                        
                            <h3 class="footer-04__title">Follow Us:</h3>
                        
                        <div class="footer-04__social_box">
                            

<div class="social-buttons  social-buttons--left ">
<ul class="social-buttons__list">
<li class="social-buttons__item"><a class="social-buttons__link social-buttons__link--facebook" href="https://www.facebook.com/supercard" target="_blank"><img loading="lazy" class="social-buttons__icon" alt="facebook icon" src="./index_files/facebook.svg"></a></li><li class="social-buttons__item"><a class="social-buttons__link social-buttons__link--twitter" href="https://www.twitter.com/supercard" target="_blank"><img loading="lazy" class="social-buttons__icon" alt="twitter icon" src="./index_files/x.svg"></a></li><li class="social-buttons__item"><a class="social-buttons__link social-buttons__link--linkedin" href="https://www.linkedin.com/company/supercard" target="_blank"><img loading="lazy" class="social-buttons__icon" alt="linkedin icon" src="./index_files/linkedin.svg"></a></li>
</ul>
</div>

                        </div>
                    </div>
                
            </div>
        </div>
        
            
                

<div class="footer-04__bottom">
<div class="container">
<div class="footer-04__bottom_wrapper footer-04__bottom_wrapper--center">
    
        



    
    <ul class="footer-04__bottom_links  text-white ">
        

  <li class="footer-04__bottom_links_item">
    <a target="_blank" href="./privacy-policy.html" class="footer-04__bottom_link footer-04__bottom_item">
      Privacy Policy
    </a>
  </li>

        

  <li class="footer-04__bottom_links_item">
    <a target="_blank" href="./terms-and-conditions.html" class="footer-04__bottom_link footer-04__bottom_item">
      Terms of Service
    </a>
  </li>

        

  <li class="footer-04__bottom_links_item">
    <a target="_blank" href="./refund-policy.html" class="footer-04__bottom_link footer-04__bottom_item">
      Refund Policy
    </a>
  </li>

        

        
    </ul>
</div>
</div>
</div>


            
        
        
    </div>
</footer>
</div>
`;